(() => {
  ...
  function renderCommercial(data) {
    ...
    data.sow_effort_plan.forEach(item => rollups.set(item.workstream, (rollups.get(item.workstream) || 0) + Number(item.hours)));
    return `<div class="commercial-hero"><div><span class="eyebrow">Ready for written authorization</span><h2>$3,200 fixed fee</h2><p>40-hour absolute effort ceiling · $1,600 start payment · $1,600 final payment · one consolidated review round.</p></div><button class="button button--primary" type="button" data-authorize>Authorize project</button></div>${table(['Commercial term', 'Value', 'Status or trigger'], data.commercial.map(item => [item.term, item.value, item.status]))}<div class="principle"><span>Assignment control</span><strong>40 planned hours is the absolute included ceiling.</strong><p>The fixed fee remains controlling. Work pauses before overrun; any further effort requires written change authorization.</p></div><h3 class="subhead">Effort allocation by deliverable</h3>${table(['Workstream / deliverable', 'Planned hours'], [...rollups].map(([name, hours]) => [name, hours.toFixed(1)]))}<div class="two-col"><article class="card"><h3>Included scope</h3><ul><li>Standard project and sheet setup</li><li>Existing floor plan</li><li>Proposed floor plan</li><li>Site and area plan</li><li>Integrated QA and check set</li><li>One consolidated BDPC review round and final issue</li></ul></article><article class="card"><h3>Excluded or separately authorized</h3><ul><li>Survey, architecture, engineering, code, zoning, permit, structural, MEP, civil, or life-safety certification</li><li>Concealed-condition verification</li><li>Effort beyond the 40-hour ceiling or services beyond the included round: $90/hour with written authorization</li><li>Future automation or standards-library development</li></ul></article></div><h3 class="subhead">Production clock starts only after all four gates</h3>${table(['Sequence', 'Kickoff gate', 'Status', 'Owner', 'Requirement'], data.kickoff_gates.map(item => [item.sequence, item.gate, { badge: item.status }, item.owner, item.requirement]))}<div class="actions"><a class="button button--secondary" href="/bdpc/sow/">Open print-optimized SOW and full work plan</a><a class="button button--secondary" href="/bdpc/reports/data/cad-effort-plan.csv" download>Download assignment CSV</a></div>`;
  }
